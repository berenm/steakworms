...
void *ISteamClient_GetInterface(void *_this, HSteamPipe pipe, const char *version) {
  ...
  void *itf = svabi::ISteamClient_GetInterface(_this, pipe, version);
...
void *ISteamClient_GetUserInterface(void *_this, HSteamUser user, HSteamPipe pipe, const char *version) {
  ...
  void *itf = svabi::ISteamClient_GetUserInterface(_this, user, pipe, version);
...
__cdecl void *_CreateInterface(const char *version, uint32_t *result) {
  ...
  if (std::strcmp("SteamClient017", version) == 0) {
    ...
    auto client = new _ISteamClient{&msabi::isteam_client_017_vtbl};
    debug("return %p", client);
    return nullptr;
  } else if (std::strcmp("SteamClient014", version) == 0) {
    auto client = new _ISteamClient{&msabi::isteam_client_014_vtbl};
    debug("return %p", client);
    ...
    auto client = new _ISteamClient{&msabi::isteam_client_007_vtbl};
    debug("return %p", client);
    return client;
  }
